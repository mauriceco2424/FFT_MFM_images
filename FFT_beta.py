...
def plot_results(img_original, img_subtracted, pow_FFT, log_FFT, shell_mask, theta_rads,
                 ...
                 peak_circle_radius, colormap, angle_info, output_filename, show_angle_mask,
                 peak_angle_deg):
    ...
    if plus_q_peak is not None and peak_angle_deg is not None:
        ...
        # Arrow (still conditional if you want)
        if show_angle_mask:
            axes[2].arrow(0, 0, px, py, color='orange', width=0.5, head_width=2, length_includes_head=True)
            angle_label = f"{peak_angle_deg:.1f}°"
            axes[2].text(px * 1.1, py * 1.1, angle_label,
                        color='orange', fontsize=9, ha='right', va='center',
    ...
    axes[2].axvline(x=0, color='white', linestyle='--', linewidth=1)

    axes[2].set_xlim(-window_width//2, window_width//2)
    axes[2].set_ylim(-window_height//2, window_height//2)
    axes[2].text(0.05, 0.95, angle_info, transform=axes[2].transAxes, color='white', fontsize=9, verticalalignment='top', bbox=dict(facecolor='black', alpha=1, edgecolor='none'))
    ...
    if plus_q_peak is not None and peak_angle_deg is not None:
        ...
        # Arrow (still conditional if you want)
        if show_angle_mask:
            axes[3].arrow(0, 0, px, py, color='orange', width=0.5, head_width=2, length_includes_head=True)
            angle_label = f"{peak_angle_deg:.1f}°"
            axes[3].text(px * 1.1, py * 1.1, angle_label,
    ...
    axes[3].axvline(x=0, color='white', linestyle='--', linewidth=1)

    axes[3].set_xlim(-window_width//2, window_width//2)
    axes[3].set_ylim(-window_height//2, window_height//2)
    axes[3].text(0.05, 0.95, angle_info, transform=axes[3].transAxes, color='white', fontsize=9, verticalalignment='top', bbox=dict(facecolor='black', alpha=1, edgecolor='none'))
...
def main(image_path, inner_radius, outer_radius, theta_min, theta_max,
         ...
         peak_circle_radius, colormap, output_filename,
         show_angle_mask, threshold_ratio):
    ...
    plot_results(img_original, img_subtracted, pow_FFT, log_FFT, shell_mask, theta_rads,
                inner_radius, outer_radius, window_width, window_height, plus_q_peak,
                peak_circle_radius, colormap, angle_info, output_filename, show_angle_mask,
                peak_angle_deg if plus_q_peak is not None else None)

if __name__ == "__main__":
    # Adjustable parameters
    image_path = '1to1_inside.png'
    output_filename = 'fft_1to1_inside.png'
    inner_radius = 10
    outer_radius = 20
    theta_min = 90
    theta_max = 180
    ...
    dc_radius = 5
    axis_width = 2
    peak_circle_radius = 3
    colormap = 'viridis'
    show_angle_mask = True
    threshold_ratio = 0.5  
